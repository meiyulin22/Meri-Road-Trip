...
If the user clearly says they want to go to a newly named destination, propose trip_state_update with the user's own destination expression; the new destination can be resolved after it becomes TripState. Geographic ambiguity does not make the user's update intent unclear. Never replace the user's expression with a candidate or mark a candidate confirmed without their choice.
