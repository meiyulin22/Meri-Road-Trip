/**
 * Cult UI PixelHeading (character variant), adapted from:
 * https://github.com/nolly-studio/cult-ui/blob/main/apps/www/registry/default/ui/pixel-heading-character.tsx
 * The project uses CSS modules, so only the font/style class mapping and
 * motion limits are adapted; the pixel-font distribution logic is retained.
 */
...
import styles from "./pixel-heading-character.module.css";
...
/** Map short name to the matching pixel font. */
...
/** Map short name to the fallback fonts for isolated characters. */
...
export interface PixelHeadingProps extends React.ComponentProps<"h1"> {
   ...
   */
  autoPlay?: boolean;
  /** Stop hover/focus animation after this many full five-font cycles. */
  interactionCycles?: number;
  /**
...
export function PixelHeading({
  ...
  const startCycling = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      elapsed += TICK_MS;
      if (interactionCycles && elapsed >= cycleInterval * FONT_COUNT * interactionCycles) {
        if (intervalRef.current) clearInterval(intervalRef.current);
        intervalRef.current = null;
        setMsElapsed(0);
        setIsActive(false);
  ...
  }, [cycleInterval, interactionCycles]);
    ...
    >
      ...
      {showLabel && (
        <output
          ...
          className={cn(
            styles.label,
            isActive || autoPlay ? styles.labelVisible : styles.labelHidden
          )}
        >
          {activeLabel}
        </output>
